...
async function ridecountImportLoad (params, authLoaded) {
  ...
  $('#wizardFormNext').on('click', async function () {
    ...
    let selectedService, ridecountcomUsername
    switch (currentPage) {
      case 'welcome':
        console.log('Next clicked on welcome page')
        selectedService = $('#wizardFormTabWelcome')
        ...
        if (selectedService) {
          ...
          window.alert(
            'Please select which service or app you want to bring your trips over from'
          )
        }
        break
      case 'ridecountcomUser':
        console.log('Next clicked on ridecount.com username input page')
        ridecountcomUsername = $('#ridecountcomUsername').val()
  ...
  $('#wizardFormFinish').on('click', async function () {
    ...
    let ridecountcomUsername
    switch (currentPage) {
      case 'ridecountcomUserConfirm':
        console.log(
          'Finish button clicked on ridecount.com username confirm page'
        )
        ridecountcomUsername = $('#ridecountcomUsername').val()
        try {
          ...
          console.log(
            `Requested migration for username: ${ridecountcomUsername}`
          )
...
const wizardPage = async function (page) {
  ...
  let existingRequestQuery, ridecountcomUsername
  switch (page) {
    case 'welcome':
      existingRequestQuery = await window.db
      ...
      if (!existingRequestQuery.empty) {
        console.log(
          'There is an existing migration request open for this user'
        )
